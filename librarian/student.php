									...
									<ul class="nav nav-tabs nav-overflow header-tabs">
										<li class="nav-item">
											<a href="#!" class="nav-link text-nowrap active">
												All Students <span class="badge rounded-pill bg-soft-secondary"><?php echo $brow - 1 ?></span>
											</a>
										</li>
									</ul>
								</div>
										...
										<table class="table table-sm table-hover table-nowrap card-table">
											<thead>
												<tr>
													...
													<th>
														<a class="list-sort text-muted" data-sort="item-email">Contact</a>
													</th>
													<th>
														<a class="list-sort text-muted" data-sort="item-phone">Id</a>
													</th>
													<th>
														<a class="list-sort text-muted" data-sort="item-score">Total Borrow</a>
													</th>
													<th>
											...
											<tbody class="list font-size-base">
												...
												while ($row = mysqli_fetch_assoc($bqurres)) {
													$ttid = $row['Username'];
													$qur = "SELECT * FROM borrow_book WHERE user_id = '$ttid'";
													$qurres = mysqli_query($conn, $qur);
													$qurrow = mysqli_fetch_assoc($qurres);
													$row1 = mysqli_num_rows($qurres);
												?>
													...
													<tr>
														...
														<td>
															<!-- Email -->
															<span class="item-email text-reset"><?php echo $row['contact']; ?></span>
														</td>
														<td>
															<!-- Phone -->
															<span class="item-phone text-reset"><?php echo $row['Username']; ?></span>
														</td>
														<td>
															<!-- Badge -->
															<span class="item-score text-reset"><?php echo $row1; ?></span>
														</td>
														<td>
